...
SHELLCOMMANDENTRY gs_vstCommandTable[] =
{
		...
		{"rdtsc", "Read Time Stame Counter", kReadTimeStampCounter},
...
void kTestTask(void)
{
	...
	while(1){
		kPrintf("[%d] This message is from kTestTask. Press and key to switch""kConsoleShell~!!\n", i++);
		kGetCh();
	...
	for(j=0; j<10; j++){
		...
		for(i=0; i<qwMemorySize/2; i++){
			if(pbAllocationBuffer[i] != pbAllocationBuffer[i + (qwMemorySize / 2)]){
				kPrintf("Task ID[0x%Q] Verfiy Fail\n", pstTask->stLink.qwID);
				kExitTask();
			}
		}
